...
Description: This program calculates the distance between two points with given coordinates.
*/
#include <stdio.h>
#include <math.h>
...
int main() {
    ...
    // Calculate the average position
    coordinate average = averageposition(points, count);
    printf("Average position: (%.2f, %.2f)\n", average.x, average.y);

    // Calculate distances between each point and the average
    for (int i = 0; i < count; i++) {
        float distance = Distance(points[i], average);
